...
"""
    ...
    try:
        send_email(subject, [user.email], body=body, html=html)
        current_app.logger.info("Password reset email sent to %s", user.email)
        return True, None
    except ClientError as ce:
        code = ce.response.get("Error", {}).get("Code")
        msg = ce.response.get("Error", {}).get("Message")
        current_app.logger.warning("SES ClientError sending reset to %s: %s - %s", user.email, code, msg)
        # Optional fallback to DEBUG_TO so you can still click the link
        ...
        current_app.logger.exception("Unexpected error sending reset email")
